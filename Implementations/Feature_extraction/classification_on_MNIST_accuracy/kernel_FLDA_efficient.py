...
class Kernel_FLDA:
    ...
    def fit(self, X, y):
        ...
        for class_index in range(n_classes):
            ...
            N = N + K_c.dot(np.eye(n_samples_of_class) - (1 / n_samples_of_class) * np.ones(
                (n_samples_of_class, n_samples_of_class))).dot(K_c.T)
        # ------ embedding:
        epsilon = 0.01  # --> to prevent singularity of matrix N
        ...
        if self.n_components is not None:
            ...
            self.U = eig_vec[:, :n_components]
        else:
            self.U = eig_vec[:, :n_classes - 1]

    def transform(self, X, y=None):
        Kernel_train_input = pairwise_kernels(X=self.X_train, Y=X, metric=self.kernel)
        X_transformed = ((self.U).T).dot(Kernel_train_input)
